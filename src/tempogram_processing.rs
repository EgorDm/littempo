	      ...
	      B: Dim
{
	...
	let spe = (spe / spe_max).clamp(thresh, 1.);
	...
	let norm_filter_f_slice_flipped = norm_filter_f_slice.flip_cols();
	...
	for (bin, mut novelty_curve) in bins.as_row_slice_iter().zip(bands_novelty_curve.as_row_slice_mut_iter()) {
		...
		// Calculate band diff
		let mut band_krn = ContainerRM::zeros(band_data.row_dim(), Dynamic::new(diff_len_half * 2 + band_data.col_count()));
		band_krn.slice_cols_mut(diff_len_half..diff_len_half+band_data.col_count()).copy_from(&band_data);
		...
		for mut row_band_diff in band_diff.as_row_slice_mut_iter() {
			row_band_diff /= &norm_curve;
		}
